...
cor_mat <- list.files("output/fastspar/output_files", pattern = "cor", full.names = T)

# Iterate over each dataset
network_props_biomes <- furrr::future_map(cor_mat, function(cor_mat) {
...
network_props_combined %>%
  ...
  ggplot(aes(x = r_val, y = modularity, col = New_Type)) +
    geom_point() +
    geom_smooth(se = T) +
    scale_color_manual(values = c(marine, soil)) +
    facet_wrap(~Biome, ncol = 1) +
    theme_bw() +
    labs(y = "Network modularity", x = "SparCC r-value") +
    theme(legend.position = "none")

ggsave("figs/corco_modularity_fixy_soil_marine.png", width = 3, height = 14/4+0.3, dpi = 300)
...
ggsave("figs/corco_clustercoef_fixy_soil_marine.png", width = 3, height = 14/4+0.3, dpi = 300)
...
ggsave("figs/corco_avgmember_fixy_soil_marine.png", width = 3, height = 14/4+0.3, dpi = 300)
...
ggsave("figs/corco_numberofcluster_fixy_soil_marine.png", width = 4.8, height = 14/4+0.3, dpi = 300)
...
network_data_combined %>%
  ggplot(aes(x = weight, fill = New_Type)) +
    ...
    scale_x_log10() +
    scale_fill_manual(values = c(marine, soil)) +
    facet_wrap(~Biome, ncol = 1) + 
    theme_bw() +
    theme(legend.position = "none") +
    labs(y = "Density distribution", x = "SparCC r-values")

ggsave("figs/corco_distribution_fixy_soil_marine.png", width = 3, height = 14/4+0.3, dpi = 300)
