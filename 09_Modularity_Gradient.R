...
datalist_Combined <- import_data("../Archive/Comparison_Paper/data/Combined_Reduced/", kingdom = "Prok")
...
datalist_Mendota <- import_data("../Lake_Mendota/data/Count_Data/Complete/", kingdom = "Prok")
...
datalist_cluster_Mendota$Count_Data <- datalist_cluster_Mendota %>%
  ...
  .$Count_Data %>%
  left_join(., dplyr::rename(clustering_datasets_0.4[[9]]$Cluster_Tab, "OTU_ID" = "ASV"), by = "OTU_ID") %>%
...
p1_Combined <- datatable_cluster_Combined %>%
  ...
  ggplot(., aes(x = Sample_ID, y = Abundance*100, fill = Group)) +
  geom_bar(stat = "identity", position = "stack") +
  scale_fill_manual(values = cbbPalette[1:max(datatable_cluster_Combined$Group)]) +
  theme_bw() +
  labs(x = "Temperature (°C)", y = "Rel. Abundance (%)", fill = "Cluster") +
  theme(axis.text.x = element_blank())
...
network_properties_Mendota <- purrr::map_df(datalist_Mendota$Meta_Data$Sample_ID, function(x) {
  ...
  network_tmp <- induced_subgraph(clustering_datasets_0.4[[9]]$Network, 
                                  vids = names(V(clustering_datasets_0.4[[9]]$Network)) %in% OTUs_tmp)
...
network_properties_Australia <- purrr::map_df(datalist_Australia$Meta_Data$Sample_ID, function(x) {
  ...
  network_tmp <- induced_subgraph(clustering_datasets_0.4[[14]]$Network, 
                                  vids = names(V(clustering_datasets_0.4[[14]]$Network)) %in% OTUs_tmp)
  modularity_tmp <- modularity(network_tmp, membership = vertex_attr(network_tmp, name = "Cluster"))
...
p2_Mendota <- datatable_cluster_Mendota %>%
  ...
  geom_point() +
  geom_smooth(method = "lm", formula = y ~ poly(x, 3), se = F, col = "darkred", lwd = 1.3) +
  theme_bw() +
